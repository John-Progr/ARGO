...
import { PriceCard, type PriceEntry } from '@/components/PriceCard'
import { RiskScore, type RiskData } from '@/components/RiskScore'
...
export default function Dashboard() {
  ...
  }, [verdict, jobId]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 dark:bg-black dark:text-white p-6 transition-colors duration-300">
      <header className="flex items-center justify-between mb-8 border-b border-zinc-200 dark:border-zinc-800 pb-4">
        <div>
          <h1 className="text-lg font-semibold tracking-tight">Autonomous Risk Guardian</h1>
          <p className="text-xs text-zinc-500 font-mono mt-0.5">Sui DeFi · Live Oracle Monitor</p>
        </div>
        <div className="flex items-center gap-4 text-xs font-mono">
          {oracleError ? (
            <span className="text-red-400">⚠ {oracleError}</span>
          ) : (
            <>
              <span className="h-2 w-2 rounded-full bg-green-400 animate-pulse" />
              <span className="text-zinc-500 dark:text-zinc-400">LIVE · {lastSeen}</span>
            </>
          )}
          <button
            onClick={() => setIsDark(!isDark)}
            className="px-2 py-1 rounded border border-zinc-300 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400 hover:border-zinc-500 dark:hover:border-zinc-500 transition-colors"
          >
            {isDark ? 'Light' : 'Dark'}
          </button>
        </div>
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="grid grid-cols-1 gap-3">
            <BtcWidget />
            {SYMBOLS.map(sym => (
              <PriceCard key={sym} symbol={sym} data={data?.prices[sym] ?? null} />
            ))}
          </div>
        </div>
              ...
              <div className="flex items-center gap-2 text-sm font-mono">
                {executing ? (
                  <>
                    ...
                    <span className="text-yellow-400">Auto-submitting to Sui {process.env.SUI_NETWORK ?? 'testnet'}…</span>
                  </>
                ) : txResult ? (
                  <>
            ...
            <div className="bg-zinc-100 border border-zinc-200 dark:bg-zinc-900 dark:border-zinc-800 rounded-xl p-5 flex items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <p className="text-xs text-zinc-500 dark:text-zinc-600">
                  Resets LTV to 75%, lifts any pause, and emits a <code className="text-zinc-400">DaoOverrideApplied</code> event.
                </p>
                {daoResult ? (
                  <a
